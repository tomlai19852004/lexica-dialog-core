...
interface MessageRepository {
  ...
  findByUniAndSenderIdAndIssueId(uni: string, senderId: string, issueId: string): Promise<BaseMessage[]>;
...
interface NlpService {
  analyse(message: string, uni: string, intentMemoriesFeatures?: List<IntentMemoryFeatures>): Promise<List<BotCommand>>;
}

interface SenderInfoRepository {
  create(senderInfo: SenderInfo): Promise<SenderInfo>;
  findOneByUniAndMessengerAndSenderId(uni: string, messenger: string, senderId: string): Promise<SenderInfo | null>;
}
