...
import { Message, Response, ResponseType } from 'lexica-dialog-model/dist/Intent';
...
function intentResponseToBotResponse(
  ...
  if (response.type === ResponseType.TEXT && !isNil(response.messages)) {
    ...
    const responses = message.split('[^LEXICA^]').map(msg => ({
      message: msg,
...
function extractMessage(messages: Message[], feature: Map<string, string>, locale: string): string {
...
function toConfigMap(configs: Config[]): Map<string, Config> {
  return configs.reduce((map, config) => map.set(config.key.toString(), config), Map<string, Config>());
}
